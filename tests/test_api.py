...
def test_api_endpoints_return_payloads():
    ...
    swings_resp = client.get("/api/swings?limit=50&price_threshold=0.05&scale_id=macro")
    assert swings_resp.status_code == 200
    swings = swings_resp.json()
    assert "swings" in swings
    assert swings["count"] >= 1
    assert swings.get("scale_id") == "macro"

    scenarios_resp = client.get("/api/scenarios?limit=50&max_scenarios=5&price_threshold=0.05&scale_id=micro")
    ...
    if scenarios["scenarios"]:
        assert "details" in scenarios["scenarios"][0]
        assert "wave_box" in scenarios["scenarios"][0]
